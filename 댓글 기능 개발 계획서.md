...
### 0. API 수정 요청 (BE 개발자 협업) ⚠️

프론트엔드 개발 중 API 응답에 필요한 정보가 누락되었거나 수정이 필요한 경우, **BE 개발자에게 수정을 요청할 수 있습니다.**

#### 현재 요청 필요한 사항:

1. **댓글 수정 API (PUT /api/v1/comment/{commentId})**
   - 현재: 응답 없음 (void)
   - 요청: 수정된 댓글 정보 반환 필요
   - 필요한 응답 형식:
     ```typescript
     {
       id: string;
       content: string;
       updatedAt: string; // ⭐ 중요: 수정된 시간 포함 필요
     }
     ```
   - 이유: 수정 후 UI에 "수정됨" 표시를 위해 `updatedAt` 정보 필요

2. **댓글 목록 조회 API (GET /api/v1/display/trend/{trendId}/item/{itemId}/comment)**
   - 현재: `CommentItem`에 `updatedAt` 포함 여부 확인 필요
   - 요청: 각 댓글의 `updatedAt` 필드 포함 (이미 구현되어 있을 수 있음)
   - 이유: 댓글 목록에서 수정된 댓글 표시

#### API 수정 요청 프로세스:

1. 필요한 정보와 이유를 명확히 문서화
2. BE 개발자와 협의
3. API 스펙 수정 합의
4. 프론트엔드 타입 정의 업데이트 ([src/types/comment.ts](src/types/comment.ts))
5. API 서비스 코드 업데이트 ([src/services/api/comment.ts](src/services/api/comment.ts))
...
**버전**: 1.3 (수정: API 수정 요청 가이드 추가, updatedAt 필드 명시)
