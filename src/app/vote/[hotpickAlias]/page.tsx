...
export { generateStaticParams } from '@/app/vote/[hotpickAlias]/params';
export { generateMetadata } from '@/app/vote/[hotpickAlias]/metadata';
  ...
  try {
    ...
    return (
      <HydrationBoundary state={dehydrate(queryClient)}>
        <VoteContent hotpickAlias={hotpickAlias} data={hotpickData ?? undefined} />
      </HydrationBoundary>
    );
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('[VotePage] Failed to fetch hotpick data:', error);
    // 서버에서 실패해도 클라이언트에서 재시도
  }

  // 실패 시 빈 데이터로 렌더링 (클라이언트에서 재시도)
  return <VoteContent hotpickAlias={hotpickAlias} />;
}
