...
비로그인 서비스의 핵심 경험(투표)은 유지하면서, 댓글·좋아요·공감은 로그인 유저 전용으로 전환한다.

### 목표

- 카카오 소셜 로그인(카카오 전용)
- 비로그인 투표 이력을 최초 1회 계정에 병합
- 댓글/좋아요/공감은 로그인 필수로 전환하여 자연스러운 가입 유도
...
유저 → 로그인 버튼 클릭
  ...
  → /auth/kakao/callback 페이지 ('use client'):
    ...
    2. POST /api/auth/kakao { code, redirectUri } 호출
       - redirectUri는 환경변수 NEXT_PUBLIC_KAKAO_REDIRECT_URI 사용 (카카오 앱 설정과 일치 필수)
