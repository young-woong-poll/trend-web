...
- HotPick 맞춤 이유: Layer 1에 "OO님은 유진님과 **소울 메이트**예요" 같은 긍정형 선언 카피로 MBTI식 자기 정체성 포지셔닝. 부정 뉘앙스 라벨(트러블 메이커 등)은 TOP 자리에 두지 않음.
...
- **Q2. 상태 정의**:
  ...
  - 소수답 Top 3 엠프티: **섹션 전체 숨김**. 발동 조건 = 그룹 참여자 3명+ AND 내가 답한 옵션 중 그룹 비율 ≤ 33% 질문이 1개+. 조건 미달 시 `MyExtremeAnswersSection` 미렌더.
...
- **Q3. 대안 경로**:
  ...
  - **비멤버(미참여자) 진입 시**: **Layer 1만 잠금, Layer 2는 공개**. 훈장 3카드 / 궤도 중앙 "나" 자리 / 소수답 Top 3는 희미한 실루엣 + 잠금 오버레이. Layer 2(그룹 어워드·투표 현황·이성 콘텐츠 토글)는 그대로 노출 → "나도 저런 결과 받고 싶다" 동기 강화. CTA "나도 참여하기".

- **Q4. 카피 원칙**:
  - **"그룹" 단어 전면 제거** — 2~3명 소수 비교에서도 자연스러운 톤.
  - 대체 표현: 명시적 대체어 추가 없이 그냥 뺀다. "이 비교의 소울 메이트" 같이 수식어 추가도 지양 — 그냥 "소울 메이트".
  - "싱크로율"은 기존 용어 유지.
  - "1/10명만 이 답을 골랐어요" 같은 강조 숫자는 유지.
  - 섹션 타이틀은 "나의 위치"로 통일(기존 "그룹에서 나의 위치"에서 축약).
  - 톤: 기존 존댓말.
  - **TOP 수상 라벨 선정 기준**: 2~4명 소수 비교에서도 발동 가능 + 중립-긍정 뉘앙스. 부정 뉘앙스 어워드(CONTROVERSY_MAKER/트러블 메이커, GROUP_OUTSIDER/이단아)는 Layer 1 선언 타이틀에 노출하지 않고 Layer 2 GroupAwards에서만 표시. 대표 예시: SOUL_CONNECTION(소울 메이트) — 2명부터 발동, 긍정적, 파트너 이름이 함께 공유되어 2차 공유 트리거.
  - **선언 타이틀 톤 변경**: "OOO님은 [라벨]예요" 대신 파트너 포함형도 허용 — "OOO님은 유진님과 **소울 메이트**예요".
  - **공유 동선 단일화**: FloatingCta 멤버 모드 라벨을 "친구들 초대하기" → **"공유하기"** 로 변경 (동작은 기존 링크 복사 그대로). Layer 1 내부의 "공유하기" 버튼은 제거, "결과 캡처" 버튼만 유지.
  - **FloatingCta 비멤버 모드**: "나도 참여하기" 유지.
...
[Layer 1 — 나]
  ...
  [결과 캡처 버튼]
    단일 버튼 (Layer 1 공유하기는 FloatingCta로 이동)
    canvas 스냅샷 + 훈장 이미지 1장 다운로드
    인스타 스토리/릴스용 이미지 공유 대응
...
1. **TOP 수상 (primary, 그라디언트 보더)**
   - 선정 기준: **2~4명 소수 비교에서도 발동 가능 + 중립-긍정 뉘앙스** 어워드 우선.
   - 우선순위: SOUL_CONNECTION(소울 메이트) > PEOPLES_CHAMPION(대중의 왕) > GROUP_LEADER(그룹 대장) > POLAR_OPPOSITES(극과 극).
   - **제외 대상**: CONTROVERSY_MAKER(트러블 메이커), GROUP_OUTSIDER(그룹 이단아) — 부정 뉘앙스로 Layer 1 TOP 수상 자리에 부적합. Layer 2 GroupAwards에서만 노출.
   - 부재 시: 성향 라벨로 TOP 자리 승격 (3번 슬롯 대체).
2. **케미 파트너**
   - SOUL_CONNECTION(최고 케미)을 기본 노출. TOP 수상이 이미 SOUL_CONNECTION이면 2순위 어워드 노출.
   - POLAR_OPPOSITES는 Layer 2 GroupAwards에서만 노출(카피 분기 불필요).
   - 2명 이상이면 SOUL_CONNECTION은 항상 존재 → 항상 채워짐.
3. **성향 라벨 (폴백)**
   - 답변 패턴으로 자동 생성되는 라벨. 매칭 태깅(same/different)을 활용해 답변 성향을 분류.
   - 연애 번들 예: "자유로운 탐험가" / "안정 추구형" / "감정 표현형" 등.
   - 라벨 마스터 리스트는 **번들별 하드코딩**. 별도 상수 파일: `src/constants/bundle-persona-labels.ts`.
   - **라벨 최대 글자 수 8자** — 모바일 카드 한 줄 수용. "균형잡힌 현실주의자" 같은 장문 라벨 지양.
...
| `+` / `−` / `↻` 버튼 | 줌 인/아웃/리셋                                                  |
| 첫 방문 hint         | "탭해서 자세히 보기" 톤 다운 문구, 2번째부터 localStorage로 숨김 |

**힌트 톤 결정**: "드래그해서 외곽 궤도까지 탐험해 보세요" 같은 탐험 유도 카피는 유저를 궤도 안에 머물게 만들어 공유 전환 타이밍을 놓치게 할 수 있음. 탐험보다 "여기서 더 볼 게 있다"는 최소 안내 톤으로 제한.

**훈장 카드** — 탭 시 해당 어워드 설명 팝업(기존 `GroupAwards`와 동일 패턴 이식 가능).

**결과 캡처 버튼** — `ShareCardCanvas`로 이미지 생성 → 모바일 Web Share API 우선, 실패 시 다운로드 폴백. 이미지 공유 경로(인스타 스토리/릴스 등)는 이 버튼 단독 경유. 링크 공유는 FloatingCta "공유하기"로 일원화.

**FloatingCta (화면 하단 고정)**

- 멤버: "공유하기" — 클릭 시 현재 페이지 URL을 클립보드 복사 + Toast "초대 링크가 복사되었어요" (기존 `handleCopyInvite` 로직 재사용, 라벨만 변경).
- 비멤버: "나도 참여하기" — 기존 플로우 유지.

**비멤버 모드**

- **Layer 1 잠금**: 궤도 중앙 "나" 자리는 희미한 실루엣 + "여기에 당신이 들어올 수 있어요" 툴팁. 훈장 3카드는 잠금 오버레이(`LockedSectionPreview`와 동일 스타일). 소수답 Top 3 잠금.
- **Layer 2 공개**: 그룹 어워드·투표 현황·이성 콘텐츠 토글은 멤버와 동일하게 노출. 비멤버가 "저런 결과가 나온다"를 직접 보고 참여 동기를 얻도록 설계.
- **결과 캡처 버튼 숨김**: 비멤버는 공유할 "자기 결과"가 없으므로 미노출.
...
- **비멤버는 Layer 1만 잠금, Layer 2는 공개**: 비멤버가 페이지 전체에 막히면 구경 자체가 차단되어 "나도 참여하기" 동기가 약해짐. Layer 2(그룹 어워드·투표 현황)를 공개해 "저런 결과가 나온다"를 직접 보여주고, Layer 1의 "나" 자리만 잠가서 "내 결과가 비어 있다"는 박탈감으로 참여 동기를 강화.
- **공유 동선 FloatingCta 단일화**: "공유하기"(링크 복사)와 "결과 캡처"(이미지 다운로드)는 유저 행동이 다름(단톡방 vs 인스타). 그러나 2개 버튼 나란히 = 판단 비용. FloatingCta에 주 전환 목표(링크 공유) 단일화 + Layer 1 내부에 결과 캡처만 남김. 이전 스펙 2026-04-19의 "CTA 단일화" 원칙 유지.
- **TOP 수상에서 부정 뉘앙스 어워드 제외**: "트러블 메이커" 라벨이 한국어에서 부정적으로 수용되면 공유 거부 발생. MBTI 바이럴은 라벨 대부분이 중립-긍정이기에 가능했던 현상. Layer 1 선언 자리는 "소울 메이트" 등 긍정형으로 고정.
- **소수답 엠프티 시 섹션 숨김**: 대체 콘텐츠("모두 동일 답") 로직은 Layer 2 `PickASide`와 역할 중복. 숨김이 레이아웃 불규칙보다 단순성 이득 큼.
...
- 2026-04-25: hp-ux-reviewer 피드백 반영.
  - Layer 1 공유 버튼 2개 → FloatingCta "공유하기"(링크 복사)로 단일화 + Layer 1 내부는 "결과 캡처"만 유지.
  - 비멤버 모드: Layer 2 공개 명시.
  - TOP 수상 선정 기준 — 부정 뉘앙스 어워드 Layer 1 제외. 예시를 "트러블 메이커" → "소울 메이트"로 교체.
  - 소수답 엠프티 조건 정의: 3명+ AND 비율 ≤ 33% 질문 1개+, 미달 시 섹션 숨김.
  - 궤도 첫 방문 힌트 톤 다운.
  - 성향 라벨 최대 글자 수 8자 기준 추가.
